...
\begin{verbatim}
     BOOT
      │ hardware init, WiFi, MQTT, subscribe topics
      v
     IDLE <--──────────────────────────────────────────────────┐
      │ HC89 card detected                                    │
      v                                                       │
 CARD_READING                                                 │
  ├─ RFID error              ──--> ACCESS_DENIED                │
  ├─ UID in revocation list  ──--> ACCESS_DENIED                │
  ├─ schema != 0x02          ──--> ACCESS_DENIED                │
  ├─ permission bit = 0      ──--> ACCESS_DENIED                │
  ├─ machine_active = false  ──--> MACHINE_DISABLED             │
  └─ permission bit = 1                                       │
         │ relay ON                                           │
         v                                                    │
   SESSION_ACTIVE                                             │
   ...
   TIMEOUT_WARNING                                   │        │
      │  │ time > limit                             │        │
      │  v relay OFF                                │        │
   TIMEOUT_EXPIRED ──--> SESSION_ENDED                │        │
      │                    │                        │        │
   HC89 absent             ├─ log entry             │        │
      │                    ├─ MQTT publish           │        │
      v relay OFF          └──────────────────────-->─┘        │
   SESSION_ENDED                                              │
      │ after 3s display                                      │
