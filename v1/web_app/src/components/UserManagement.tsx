...
const UserManagement: React.FC<UserManagementProps> = ({ users, setUsers, availableMachines }) => {
    ...
    const handleEditAccess = (user: User) => {
        ...
        setSelectedMachines(user.accessible_machines);
    ...
    const handleSaveAccess = async () => {
        ...
        try {
            // Update in Firebase (mock for demo)
            const usersCollection = collection(db, "users");
            const q = query(usersCollection, where("roll_number", "==", selectedUser.roll_number));
            const querySnapshot = await getDocs(q);

            if (querySnapshot.empty) return toast({
                title: "Access update failed",
                description: `Machine access update for ${selectedUser.name} failed`
            });
            ...
            setUsers(users.map(user =>
                user.roll_number === selectedUser.roll_number
                    ? { ...user, accessible_machines: selectedMachines, last_updated: new Date() }
                    : user
            ));
            ...
            toast({
                ...
                description: `Machine access updated for ${selectedUser.name}`,
            });
        } catch (error) {
            toast({
                title: "Error",
                            ...
                            <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="flex gap-1 flex-shrink-0">
                                    <Button
                                        ...
                                        className="opacity-0 group-hover:opacity-100 transition-opacity p-1 md:p-2"
                                    >
                        ...
                        <CardContent className="space-y-3 md:space-y-4">
                            ...
                            <div className="flex items-center justify-between pt-2 border-t flex-wrap gap-2">
                                <div className="flex items-center gap-2 text-xs text-gray-500">
                                    ...
                                    <span className="whitespace-nowrap">Updated {user.last_updated.toLocaleDateString()}</span>
                                </div>
                                <div className="flex items-center gap-1">
                                    <div className={`w-2 h-2 rounded-full ${user.card_id ? 'bg-green-500' : 'bg-gray-300'} flex-shrink-0`}></div>
